...
Knowledge Base: ${knowledgeBase || "N/A"}. If the user provides an answer or demonstrates a skill, provide constructive feedback. Wrap specific feedback points in [FEEDBACK] and [END FEEDBACK] tags. Keep the conversation natural and encouraging.`
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      const activeCourseContext = studentAssignedTitle || (selectedCourseFilter !== 'All Course Titles' ? selectedCourseFilter : 'General Academic & Presentation');

      const prompt = `You are a professional ChatGPT Video & Speech Translator for students at Endless Spark School.
Student's Assigned Course / Discipline: ${activeCourseContext}
...
Please respond with a well-formatted English response tailored for a student presenting in ${activeCourseContext}:
...
3. 💡 **Useful Vocabulary / Phrases**: (2-3 helpful presentation phrases relevant to ${activeCourseContext})`;
...
${script.englishText}`;
  ...
  return (
    <div className="flex flex-col min-h-full bg-gray-50 max-w-7xl mx-auto w-full p-2 sm:p-4 md:p-6 space-y-4">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {studentAssignedTitle && (
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-indigo-100 text-indigo-800 border border-indigo-200 flex items-center gap-1">
                    <GraduationCap className="w-3 h-3 text-indigo-600" /> {studentAssignedTitle}
                  </span>
                )}
              </div>
          ...
          <div className="bg-slate-900 text-white p-3 px-6 flex flex-wrap items-center justify-between gap-4 text-xs border-b border-slate-800">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="px-2.5 py-0.5 bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded-full font-bold flex items-center gap-1">
                <Check className="w-3 h-3 text-emerald-400" /> FREE: ChatGPT Video & Speech Translator, Audio & Analytics
              </span>
        ...
        {/* Tabs */}
        <div className="flex border-b border-gray-100 bg-white overflow-x-auto scrollbar-none [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <button
        ...
        {/* Tab Content Area */}
        <div className="flex-1 p-4 sm:p-6 space-y-6 bg-gray-50/50">
          ...
          {activeTab === 'translator' && (
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {studentAssignedTitle && (
                      <span className="px-2.5 py-1 bg-emerald-500/30 backdrop-blur-md border border-emerald-400/50 text-emerald-200 text-xs font-black rounded-full uppercase tracking-wider flex items-center gap-1.5">
                        <GraduationCap className="w-3.5 h-3.5 text-emerald-300" /> Course: {studentAssignedTitle}
                      </span>
                    )}
                  </div>
              ...
              {/* Student Assigned Course Banner */}
              {studentAssignedTitle && (
                <div className="bg-emerald-950/90 border border-emerald-600/60 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3 text-emerald-100 shadow-md">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 flex items-center justify-center shrink-0 font-bold">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-emerald-200/80 mt-0.5">
                        Free ChatGPT Video & Speech Translator is automatically configured for your assigned course and modules.
                      </p>
                    </div>
                  </div>
                  {selectedCourseFilter !== studentAssignedTitle && (
                    <button
                      onClick={() => {
                        setSelectedCourseFilter(studentAssignedTitle);
                      ...
                      className="px-3.5 py-1.5 bg-emerald-400 hover:bg-emerald-300 text-slate-950 text-xs font-black rounded-xl transition shadow-sm cursor-pointer"
                    >
                      Switch Back to {studentAssignedTitle}
                    </button>
                  )}
                </div>
              )}
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-purple-100 pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-black text-purple-900 uppercase tracking-wider">
                        Select Course Title & Module Script
                      </span>
                    </div>
              ...
              <div className="bg-white p-5 rounded-2xl border-2 border-emerald-400/60 shadow-md space-y-4 relative flex flex-col justify-between hover:border-emerald-500 transition">
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-gray-500 leading-relaxed">
                    Translate your native thoughts into English video scripts, practice speech pronunciation, and read course module scripts.
                  </p>
                  <ul className="text-xs space-y-2 font-medium text-gray-700 pt-1">
                    ...
                    <li className="flex items-center gap-2">
                      <Check className="w-4 h-4 text-emerald-500 shrink-0" />
                      <span>Paragraphic Script Reader & Accents</span>
                    </li>
                  </ul>
                </div>
                <button
